...
class SequentialPipeline(BasicPipeline):
    ...
    def run_with_retrieval_results(self, dataset, do_eval=True, pred_process_fun=None, retrieval_results=None, top_k=None):
        input_query = dataset.question

        pre_retrieval_results = []
        for i in range(len(input_query)):
            if top_k is not None:
                pre_retrieval_results.append(retrieval_results[i][:top_k])
            else:
                pre_retrieval_results.append(retrieval_results[i])
        # retrieval_results = self.retriever.batch_search(input_query)
        dataset.update_output("retrieval_result", pre_retrieval_results)
